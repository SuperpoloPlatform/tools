...
/* 	AssertTrue(load("/importer/mergexmls.js"),
	...
	if(convertmode == "factory")
	{
		...
		if(callbacks.Choice["mergexmls"])
		{
			...
			var arrMeshGen = [];
			var meshgobj = MESHNAME.data;
			for(var key in meshgobj){
				 for (var ikey in meshgobj[key])
					arrMeshGen.push(meshgobj[key][ikey]);
				}
			//从meshgen数组里找到是否此模型是meshgen,同时删除这个数据
			var ismeshgen = function(arr,value){
				var flag = false;
				if(arr.length > 0)
				{
					for (var i in arr) 
			...
			//copy 结点数据,其实这和meregxml一样的.
			var cpymeshnode = function(dnode, snode){   //参数均为iDocumentNode对象
				...
				for(;sci.HasNext();)     //扫描所有的子节点
				{
					...
					if(!cpymeshnode(dnode.CreateChild(tchild.type),tchild))  //检查执行是否成功
					{
						 System.exitcode = 32;
					}
				}
			...
			//meshgen.xml
			meshgenfile = VFS.Open("outputpath/meshgen.xml",1);
			var meshgenxml = new xmlDocument(); 
			var meshgwd = meshgenxml.CreateRoot();
			var meshglib = meshgwd.CreateChild(xmlDocument.NODE_ELEMENT);
			meshglib.value = "library";	
			//tworld = tworldxml.root.GetChild("world");
			var MeshObjs = tworld.GetChild("sector").GetChildren();
			for(;MeshObjs.HasNext();){
				var meshobj = MeshObjs.Next();
				if(meshobj.value == "meshobj")
				{
					var attrs = meshobj.GetAttribute();
					while(attrs.HasNext()){
						var attr = attrs.Next();
						var meshgname = attr.value;
						if(ismeshgen(arrMeshGen,meshgname))
						{	
							var meshgnode = meshglib.CreateChild(xmlDocument.NODE_ELEMENT);
							cpymeshnode(meshgnode,meshobj);
							var parwd = meshobj.parent;
							parwd.RemoveChildren(meshobj);//dele 
						}
					}
				}
			} 
			meshgenxml.Write(meshgenfile);
			meshgenfile.Flush();	
			}	
		}
